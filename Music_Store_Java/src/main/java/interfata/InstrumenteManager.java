...
public class InstrumenteManager extends JFrame {

	private ResultSet rs;
    private java.sql.Statement stmt;
	private OptiuniManager optiuni;
	private FiltrareInstrumente filtrareinstrumente;
	...
	public static void main(String[] args) {
		EventQueue.invokeLater(new Runnable() {
			public void run() {
				try {
					InstrumenteManager frame = new InstrumenteManager();
	...
	public InstrumenteManager() {
		...
		textField_5.setBounds(79, 174, 228, 20);
		...
		btnNewButton_4.addActionListener(e -> {
		    try (Connection conn = ConectareBD.connect()) {
		        String query = "INSERT INTO instrumente (instrument, tip_instrument, brand, material, culoare, pret) VALUES (?, ?, ?, ?, ?, ?)";
		        ...
		        pstmt.setDouble(6, Double.parseDouble(textField_5.getText()));
		        pstmt.executeUpdate();
		        JOptionPane.showMessageDialog(null, "Instrument adăugat cu succes!");
		    } catch (SQLException ex) {
		        JOptionPane.showMessageDialog(null, "Eroare la adăugarea instrumentului: " + ex.getMessage(), "Eroare", JOptionPane.ERROR_MESSAGE);
		    }
		});
		btnNewButton_4.setFont(new Font("Monospaced", Font.PLAIN, 11));
		btnNewButton_4.setBounds(20, 243, 89, 23);
		...
		btnNewButton_5.addActionListener(e -> {
		    try {
		        // Verificăm dacă există un instrument selectat
		        if (rs != null) {
		            // Obținem instrumentul curent (acesta va fi șters)
		            String numeAlbum = rs.getString("instrument");

		            // Creăm conexiunea și statement-ul pentru ștergere
		            try (Connection conn = ConectareBD.connect()) {
		                String query = "DELETE FROM instrumente WHERE instrument = ?";
		                PreparedStatement stmt = conn.prepareStatement(query);
		                stmt.setString(1, numeAlbum);  // Setăm parametrul pentru instrumentul care va fi șters

		                // Executăm ștergerea
		                int rowsAffected = stmt.executeUpdate();

		                // Verificăm dacă s-a șters un instrument
		                if (rowsAffected > 0) {
		                    JOptionPane.showMessageDialog(null, "Instrumentul a fost șters cu succes.");

		                    // Navigăm la următorul instruemnt din ResultSet
		                    if (!rs.next()) {
		                        // Dacă nu există un instrument următor, revenim la instruemnt anterior
		                        if (rs.previous()) {
		                            populateFields(rs);
		                        } else {
		                            JOptionPane.showMessageDialog(null, "Nu mai există instrumente.");
		                        }
		                    } else {
		                        populateFields(rs);  // Populăm câmpurile cu instrumentul următor
		                    }
		                } else {
		                    JOptionPane.showMessageDialog(null, "Nu s-a găsit instrumentul de șters.");
		                }
		            }
		        }
		    } catch (SQLException ex) {
		        JOptionPane.showMessageDialog(null, "Eroare la ștergerea instrumentului: " + ex.getMessage(), "Eroare", JOptionPane.ERROR_MESSAGE);
		    }
		});
		...
		btnNewButton_5.setBounds(118, 243, 89, 23);
		...
		btnNewButton_6.addActionListener(new ActionListener() {
			...
			public void actionPerformed(ActionEvent e) {
                ...
                if (filtrareinstrumente == null) {
                	Connection conn;
                	try {
						conn = ConectareBD.connect();
						filtrareinstrumente = new FiltrareInstrumente(conn);
                ...
                filtrareinstrumente.setVisible(true); // Afișăm interfața
                dispose();
			}
		});
		btnNewButton_6.setFont(new Font("Monospaced", Font.PLAIN, 11));
		btnNewButton_6.setBounds(217, 243, 89, 23);
		...
		btnNewButton_7.setBounds(319, 173, 89, 23);
		...
		btnNewButton_8.setBounds(319, 243, 89, 23);
